...
export default function Booking() {
  ...
  const { warnMessage, infoMessage, errorMessage, successMessage } =
  ...
  const closeComments = () => {
    dispatch(tripsActions.comments({ comments: null }));
    dispatch(tripsActions.openTripRequest({ value: null }));
    setComments(false);
  };
  ...
  React.useEffect(() => {
    if (!trips || (trips.length == 0 && searchValue.length == 0)) {
      ...
      setTimeout(() => {
        dispatch(alertActions.error({ message: "none" }));
      }, 10000);
    }
  ...
  const getAllLocation = (locationId, index) => {
    if (getLocation) {
      if (!trips[index]["location"]) {
        axios
          .get(`${process.env.API_URL}/location/${locationId}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then((res) => {
            dispatch(
              tripsActions.updateTripRequests({
                index,
                property: "location",
                value: res.data.data?.location.locationName,
              })
            );
  ...
  const getAllComments = (tripId, index) => {
    if (getComments) {
      axios
        .get(`${process.env.API_URL}/user/trip/${tripId}/comments`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
  ...
  const searchTerm = (e) => {
    ...
    if (e.target.value.length == 0)
      return dispatch(tripsActions.getTripRequests({ trips: null }));
    if (e.target.value.length >= 3) {
      setSearchLoading(true);
      axios
        .get(`${process.env.API_URL}/search/${searchValue}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then(async (res) => {
          try {
            let allTrips = await axios.get(
              `${process.env.API_URL}/user/trip/get`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
            ...
            let searchFilter = allTrips.data.data.filter((trip) =>
              retrievedData.includes(trip.id)
            );
            dispatch(tripsActions.getTripRequests({ trips: searchFilter }));
  ...
  return (
    <Container maxWidth="xl" className="book-container">
      <Stack sx={alertStyle} spacing={2}>
        {warnMessage && warnMessage != "none" && (
          <WarnAlert message={warnMessage} />
        )}
        {infoMessage && infoMessage != "none" && (
      ...
      <CommentsModal open={comments} onClose={closeComments} />
        ...
        <Grid container spacing={3}>
          <Grid item xs={8}>
            <Item className="book-grid">
              ...
              {(!trips && !errorMessage) || searchLoading ? (
                <PreLoader />
              ) : trips?.length > 0 ? (
                  ...
                  if (trip.id) {
                    ...
                    return (
                      <TripRequest
                        ...
                        openComments={() => {
                          dispatch(
                            tripsActions.openTripRequest({ value: index })
                          );
                          setComments(true);
                        }}
                        openDetails={() => {
